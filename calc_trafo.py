...
def schutz_U0(p: dict) -> dict:
    ...
    Hinweis: nicht im 7UT613 enthalten -> separates 7SJ64/zentrale Erfassung.
    """
...
def bemessung_engineering(g: dict, p: dict, umz: dict) -> dict:
    ...
    # I_k_min: bevorzugt Eingabe (DIN EN 60909-0, c=0,95), sonst Näherung 0,3 x Ik''max OS.
    Ik_min_OS = p.get("Ik_min_OS_kA", 0.0) * 1000  # A prim. OS
    if Ik_min_OS > 0:
        I_k_min        = Ik_min_OS
        I_k_min_quelle = "Eingabe (DIN EN 60909-0)"
    else:
        I_k_min        = Ik_max_OS * 0.3
        I_k_min_quelle = "Näherung 0,3 × Ik″max OS"
...
def plausibilitaet(g: dict, p: dict, b49: dict, beng: dict, umz: dict) -> list:
    ...
    # ── Spannungs-/Frequenz-/Übererregungsschutz (nur VT-Variante) ───────────
    if p.get("VT_vorhanden", False):
        U_lt   = p.get("U_lt",   K.U_lt_def)
        U_ltlt = p.get("U_ltlt", K.U_ltlt_def)
        U_gt   = p.get("U_gt",   K.U_gt_def)
        U_gtgt = p.get("U_gtgt", K.U_gtgt_def)

        # 27: U< und U<< unterhalb Nennspannung, gestaffelt
        u_lt_ok = U_ltlt < U_lt < 1.0
        add("27 Unterspannung gestaffelt unter Un", "U<< < U< < 1,0",
            f"{U_ltlt:.2f} < {U_lt:.2f} < 1,00", "OK" if u_lt_ok else "Prüfen!",
            "U< etwas unter minimaler Betriebsspannung, U<< als tiefere Schnellstufe.",
            "Regler so setzen, dass U<< < U< < 1,0 (z.B. 0,65 / 0,75).")

        # 59: U> und U>> oberhalb Nennspannung, gestaffelt
        u_gt_ok = 1.0 < U_gt < U_gtgt
        add("59 Überspannung gestaffelt über Un", "1,0 < U> < U>>",
            f"1,00 < {U_gt:.2f} < {U_gtgt:.2f}", "OK" if u_gt_ok else "Prüfen!",
            "U> über maximaler Betriebsspannung, U>> als hohe Schnellstufe.",
            "Regler so setzen, dass 1,0 < U> < U>> (z.B. 1,15 / 1,30).")

        # 27/59: Fenster konsistent (U< unter U>)
        fenster_ok = U_lt < U_gt
        add("27/59 Spannungsfenster konsistent", "U< < U>",
            f"{U_lt:.2f} < {U_gt:.2f}", "OK" if fenster_ok else "Prüfen!",
            "Unter- und Überspannungsschwellen dürfen sich nicht überschneiden.",
            "U< unter U> legen.")

        # 81: Frequenzstufen monoton (nur Checkbox nötig)
        f_lt     = p.get("f_lt",     K.f_lt_def)
        f_ltlt   = p.get("f_ltlt",   K.f_ltlt_def)
        f_ltltlt = p.get("f_ltltlt", K.f_ltltlt_def)
        f_gt     = p.get("f_gt",     K.f_gt_def)
        fN       = p.get("fN_Hz",    K.fN_default)
        f_ok = f_ltltlt < f_ltlt < f_lt < fN < f_gt
        add("81 Frequenzstufen monoton gestaffelt", "f<<< < f<< < f< < fN < f>",
            f"{f_ltltlt:.1f} < {f_ltlt:.1f} < {f_lt:.1f} < {fN:.0f} < {f_gt:.1f}",
            "OK" if f_ok else "Prüfen!",
            "Unterfrequenzstufen absteigend, f> über Nennfrequenz (7UT613 Hdb. 2.16.2).",
            "Stufenwerte korrigieren, z.B. 47,5 / 48,0 / 49,0 / [50] / 51,5 Hz.")

        # 24: U/f-Stufen gestaffelt
        if p.get("VT_Prim_kV", 0.0) > 0:
            Uf_gt   = p.get("Uf_gt",   K.Uf_gt_def)
            Uf_gtgt = p.get("Uf_gtgt", K.Uf_gtgt_def)
            uf_ok = 1.0 <= Uf_gt < Uf_gtgt
            add("24 Übererregung gestaffelt", "1,0 <= U/f > < U/f >>",
                f"{Uf_gt:.2f} < {Uf_gtgt:.2f}", "OK" if uf_ok else "Prüfen!",
                "Warnstufe (dauernd zul. Induktion) unter Schnellauslösung (7UT613 Hdb. 2.11.2).",
                "U/f > unter U/f >> legen (z.B. 1,10 / 1,40).")
...
def reserveschutz(g: dict, umz: dict, b49: dict, p: dict) -> list:
    """Zuordnungstabelle Reserveschutz (UAMZ-PH / -3I0 / -ERD / Überlast)."""
    uos = p.get("UN_OS_kV", 0.0)
    uus = p.get("UN_US_kV", 0.0)
    return [
        {"funktion": "UAMZ-PH 1 (Adr.420)",
         "ziel": f"OS-Seite — Speiseseite / Netzanschluss ({uos:.0f} kV)",
         "I_sek": f"{umz['OS']['I_sek']:.4f}", "t": f"{umz['t_I']:.2f}",
         "IHH_sek": f"{umz['OS']['IHH_sek']:.4f}", "tHH": f"{umz['t_IHH']:.2f}",
         "adresse": "Stufen 2011/2014; T 2013/2016"},
        {"funktion": "UAMZ-PH 1 (Adr.420)",
         "ziel": f"US-Seite — Lastseite / Trafo-Abgang ({uus:.0f} kV)",
         "I_sek": f"{umz['US']['I_sek']:.4f}", "t": f"{umz['t_I']:.2f}",
         "IHH_sek": f"{umz['US']['IHH_sek']:.4f}", "tHH": f"{umz['t_IHH']:.2f}",
         "adresse": "Stufen 2011/2014; T 2013/2016"},
        {"funktion": "UAMZ-3I0 1 (Adr.422)",
         "ziel": f"OS-Seite — Erdstrom-Erfassung (Holmgreen / Sternpunkt-CT, {uos:.0f} kV)",
        ...
        {"funktion": "Überlast 49 (Adr.442)",
         "ziel": f"OS-Seite — Trafo-Wicklung (thermisch, Messung {uos:.0f} kV)",
...
def geraeteempfehlung_erdschluss(p: dict) -> dict:
    ...
    if "Isoliert" in stp:
        geraet = ("7SJ64 mit Erdschlusspaket (wattmetrisch / sin-phi) "
        ...
        geraet = "7SJ64 mit cos-phi-Erfassung + Pulsortung"
...
# ── 4.17  Spannungs-/Frequenzschutz (nur VT-Variante) ───────────────────────

def _vt_verfuegbar(p: dict) -> bool:
    """Volles VT-Gate: Spannungseingang vorhanden UND VT-Verhältnis eingegeben."""
    return bool(p.get("VT_vorhanden", False)) and p.get("VT_Prim_kV", 0.0) > 0


def schutz_27_59(p: dict) -> dict:
    """
    ANSI 27/59 Unter-/Überspannung (Mitsystem, verkettete Werte).
    Voraussetzung: Gerätevariante mit VT + VT-Verhältnis (Volt-Sekundärwerte).
    Ansprechwerte bezogen auf verkettete Sekundärnennspannung UnS = VT_Sek_V.
    """
    if not _vt_verfuegbar(p):
        grund = ("Gerätevariante ohne Spannungsmesseingang -> 27/59 nicht möglich."
                 if not p.get("VT_vorhanden", False) else
                 "Spannungseingang vorhanden, aber kein VT-Verhältnis (VT prim.) eingegeben.")
        return {"aktiv": False, "grund": grund}

    UnS  = p["VT_Sek_V"]            # verkettete Sek.-Nennspannung [V]
    UnP  = p["VT_Prim_kV"]          # verkettete Prim.-Nennspannung [kV]
    U_lt   = p.get("U_lt",   K.U_lt_def)
    U_ltlt = p.get("U_ltlt", K.U_ltlt_def)
    U_gt   = p.get("U_gt",   K.U_gt_def)
    U_gtgt = p.get("U_gtgt", K.U_gtgt_def)

    def stufe(name, fak, t, adr_bez, adr_volt, adr_t):
        return {"name": name, "faktor": fak,
                "U_sek_V":  round(fak * UnS, 1),
                "U_prim_kV": round(fak * UnP, 3),
                "t_s": t, "adr_bez": adr_bez, "adr_volt": adr_volt, "adr_t": adr_t}

    return {
        "aktiv": True,
        "UnS_V": UnS, "UnP_kV": UnP,
        "stufen": [
            stufe("27  U<",  U_lt,   p.get("t_U_lt",   K.t_U_lt_def),   "5212", "5211", "5213"),
            stufe("27  U<<", U_ltlt, p.get("t_U_ltlt", K.t_U_ltlt_def), "5215", "5214", "5216"),
            stufe("59  U>",  U_gt,   p.get("t_U_gt",   K.t_U_gt_def),   "5312", "5311", "5313"),
            stufe("59  U>>", U_gtgt, p.get("t_U_gtgt", K.t_U_gtgt_def), "5315", "5314", "5316"),
        ],
        "rueckfall": p.get("U_Rueckfall", K.U_Rueckfall_def),
    }


def schutz_81(p: dict) -> dict:
    """
    ANSI 81 Frequenzschutz (4-stufig). Frequenz aus Messspannung.
    Voraussetzung: nur Spannungseingang nötig, KEIN VT-Verhältnis (Werte in Hz).
    """
    if not p.get("VT_vorhanden", False):
        return {"aktiv": False,
                "grund": "Gerätevariante ohne Spannungsmesseingang -> 81 nicht möglich."}

    return {
        "aktiv": True,
        "fN_Hz": p.get("fN_Hz", K.fN_default),
        "stufen": [
            {"name": "81U  f<",   "f_Hz": p.get("f_lt",     K.f_lt_def),     "t_s": p.get("t_f_lt",     K.t_f_lt_def),     "adr_f": "5611", "adr_t": "5641"},
            {"name": "81U  f<<",  "f_Hz": p.get("f_ltlt",   K.f_ltlt_def),   "t_s": p.get("t_f_ltlt",   K.t_f_ltlt_def),   "adr_f": "5612", "adr_t": "5642"},
            {"name": "81U  f<<<", "f_Hz": p.get("f_ltltlt", K.f_ltltlt_def), "t_s": p.get("t_f_ltltlt", K.t_f_ltltlt_def), "adr_f": "5613", "adr_t": "5643"},
            {"name": "81O  f>",   "f_Hz": p.get("f_gt",     K.f_gt_def),     "t_s": p.get("t_f_gt",     K.t_f_gt_def),     "adr_f": "5614", "adr_t": "5644"},
        ],
    }
...
def schutz_24(p: dict) -> dict:
    """
    ANSI 24 Übererregungsschutz U/f (bezogen auf Nenn-U/f).
    Voraussetzung (lt. Entscheidung): wie 27/59 an volles VT-Gate gekoppelt.
    """
    if not _vt_verfuegbar(p):
        grund = ("Gerätevariante ohne Spannungsmesseingang -> 24 nicht möglich."
                 if not p.get("VT_vorhanden", False) else
                 "Spannungseingang vorhanden, aber kein VT-Verhältnis (VT prim.) eingegeben.")
        return {"aktiv": False, "grund": grund}
    ...
    return {
        "aktiv": True,
        "Uf_gt":     p.get("Uf_gt",     K.Uf_gt_def),
        "t_Uf_gt":   p.get("t_Uf_gt",   K.t_Uf_gt_def),
        "Uf_gtgt":   p.get("Uf_gtgt",   K.Uf_gtgt_def),
        "t_Uf_gtgt": p.get("t_Uf_gtgt", K.t_Uf_gtgt_def),
        "adresse_warn":   "Adr.4302 / T 4303",
        "adresse_schnell":"Adr.4304 / T 4305",
        "hinweis_kennlinie": "Thermische Kennlinie (8 Wertepaare) Adr.4306-4313 separat.",
    }


# ── 4.18  Leistungsschalterversagerschutz 50BF ─────────────────────────────

def schutz_50BF(g: dict, p: dict) -> dict:
    """
    ANSI 50BF Leistungsschalterversagerschutz (stromflussbasiert).
    Stromschwelle als Faktor x IN je Seite. T2 (Adr.7016) ergibt sich aus
    max. Ausschaltzeit LS, Rueckfallzeit der Stromflusserfassung und Sicherheits-
    zuschlag. Zweistufig zusaetzlich T1 (Adr.7015) fuer lokale Auslösewiederholung.
    """
    if not p.get("BF_aktiv", True):
        return {"aktiv": False, "grund": "Leistungsschalterversagerschutz nicht projektiert (Adr.170)."}

    fak  = p.get("I_BF", K.I_BF_def)
    t_ls = p.get("t_LS_aus",  K.t_LS_aus_def)
    t_rf = p.get("t_RF_strom", K.t_RF_strom_def)
    t_si = p.get("t_sich_BF", K.t_sich_BF_def)
    zwei = p.get("BF_zweistufig", K.zweistufig_def)

    t_krit = round(t_ls + t_rf + t_si, 3)          # Grund-Wartezeit nach Anwurf
    if zwei:
        T1 = t_krit                                 # lokale Auslösewiederholung
        T2 = round(T1 + t_ls + t_si, 3)             # danach umliegende LS / Sammelschiene
    else:
        T1 = None                                   # Adr.7015 = ∞
        T2 = t_krit

    return {
        "aktiv": True, "zweistufig": zwei,
        "I_BF_faktor": fak,
        "I_BF_OS_prim_A": round(fak * g["IN_OS_prim"], 2),
        "I_BF_US_prim_A": round(fak * g["IN_US_prim"], 2),
        "I_BF_OS_sek_A":  round(fak * g["IN_OS_sek"], 3),
        "I_BF_US_sek_A":  round(fak * g["IN_US_sek"], 3),
        "T1_s": T1, "T2_s": T2,
        "zeitbilanz": {"t_LS_aus": t_ls, "t_Rueckfall": t_rf, "t_Sicherheit": t_si},
        "adresse": "T1 Adr.7015 / T2 Adr.7016",
    }
...
def berechne_alle(p: dict) -> dict:
    ...
    Erwartete Pflichtfelder in p:
      Sn_MVA, UN_OS_kV, UN_US_kV, uk_pct, Schaltgruppe, Stufensteller_pm,
    ...
    e2759 = schutz_27_59(p)
    e81  = schutz_81(p)
    e24  = schutz_24(p)
    ebf  = schutz_50BF(g, p)
    plaus = plausibilitaet(g, p, b49, beng, umz)
    rsv  = reserveschutz(g, umz, b49, p)
